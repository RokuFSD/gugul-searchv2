      ...
      {item?.thumbnail ? (
        <a href={item?.link} target="_blank" rel="noreferrer">
          <div className="relative">
            ...
            <p
              className="text-xs text-white bg-neutral-800 bg-opacity-70 absolute bottom-0 rounded-full m-1 w-8 text-center">{item?.duration}</p>
          </div>
        </a>) : null}
      <div className="flex flex-col justify-between w-full">
        ...
        <div className="flex flex-wrap">
          {extensions.map((extension, index) => (
            <p key={extension}
               className={`${index === extensions.length - 1 ? "basis-full" : ""} text-sm text-gray-300`}>{`${extension} `}</p>
          ))}
        </div>
      </div>
